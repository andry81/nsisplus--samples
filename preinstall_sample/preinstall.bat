...
call %%CECHO%% ^({06}c{#}^) [{0F}EN{#}] {06}Cancel{#} %%?01%% [{0F}RU{#}] {06}Îòìåíà{#}{\n}
...
%CECHO% [{0F}RU{#}] {0C}ÂÍÈÌÀÍÈÅ{#}: {06}ÑÊÐÈÏÒ ÁÓÄÅÒ ÎÁÍÎÂËßÒÜ/ÓÄÀËßÒÜ ÔÀÉËÛ ÈÍÑÒÀËËßÒÎÐÀ, ÂÛ ÍÅ ÑÌÎÆÅÒÅ ÏÎÒÎÌ ÈÕ ÂÎÑÑÒÀÍÎÂÈÒÜ{#}!{\n}
echo.
...
:REMOVE_APP_TARGET_DIR
if ^%APP_TARGET_DIR_TO_REMOVE_PTTN:~0,1%/ == ^"/ (
  call set "APP_TARGET_DIR_TO_REMOVE=%APP_TARGET_DIR_TO_REMOVE_PTTN:~1,-1%"
) else (
  set "APP_TARGET_DIR_TO_REMOVE=%APP_TARGET_DIR_TO_REMOVE_PTTN%"
)
rem if not exist "%SETUP_ROOT%\%APP_TARGET_DIR_TO_REMOVE%" exit /b 1
call :RMDIR "%%SETUP_ROOT%%\%%APP_TARGET_DIR_TO_REMOVE%%"
exit /b 0
